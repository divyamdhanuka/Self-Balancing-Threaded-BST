...
class mymap {
 private:
  ...
  void balancing(NODE* root, int begin, int end, vector<struct NODE*>& nodes,
    ...
    if (root == nullptr) {
      ...
      root->left = nodes[mid];
      nodes_toUpdate.push(
          root);  // keeping tract of nodes that havce to update nL and nR
      ...
      nodes_toUpdate.push(
          root);  // keeping track of nodes that have to update nL and nR
  ...
  void updateSizes(stack<NODE*>& nodes, bool toBalance) {
    ...
    // if there are nodes in the vector then there need to be rebalancing
    if (nodesToRebalance.size() > 0) {
      stack<NODE*> nodes_toUpdate;
      ...
      // balance the subtree
      balancing(root, 0, Nodes.size() - 1, Nodes, nodes_toUpdate);
      // update the nL and nR values of all the nodes that were changed
      updateSizes(nodes_toUpdate, false);
    }
    return;
  }
  ...
  void _deepCopy(NODE* other) {
    if (other == nullptr)
      ...
      insert(other->key, other->value, false);
      _deepCopy(other->left);
      _deepCopy((other->isThreaded) ? nullptr : other->right);
    }
  }
 ...
 public:
  ...
  //
  void put(keyType key, valueType value) { insert(key, value, true); }
